...
#define LIST_ID(NAME)                                                          \
  int lId = static_cast<int>(film[instructions[currentInstruction + 1]]);      \
  if (lId < 0 || lId >= static_cast<int>(matrix.size())) {                     \
    fprintf(stderr,                                                            \
            "\n" NAME                                                          \
            ": Matrix Error: list index %d out of bounds (have %zu lists)!\n", \
            lId, matrix.size());                                               \
    if (debugFile) {                                                           \
      fprintf(stderr, "%s\nCode: %s\n",                                        \
              lines.at((currentInstruction / STEP) * 2).c_str(),               \
...
#define ELEMENT_ID(NAME)                                                       \
  int idx = static_cast<int>(film[instructions[currentInstruction + 2]]);      \
  if (idx < 0 || idx >= static_cast<int>(matrix[lId].size())) {                \
    fprintf(                                                                   \
        stderr,                                                                \
        "\n" NAME                                                              \
        ": Matrix Error: index %d out of bounds for list %d (size %zu)!\n",    \
...
double modulo(const double num, const double base) {
  ...
  if (std::abs(r) < std::abs(base) * 1e-9)
    return 0.0;
...
void store(const std::vector<double> &filename,
...
void addToBuffer(const char c) { printBuffer.push_back(c); }
...
int main(int argc, char *argv[]) {
  ...
  static const void *labels[] = {&&_copy,
                                 &&_add,
                                 &&_subtract,
                                 &&_multiply,
                                 &&_divide,
...
// region Labels
_copy:
  film[instructions[currentInstruction + 2]] =
      film[instructions[currentInstruction + 1]];
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];

_add:
  film[instructions[currentInstruction + 3]] =
      film[instructions[currentInstruction + 1]] +
      film[instructions[currentInstruction + 2]];
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_divide:
  ...
  CHECK_NAN(film[instructions[currentInstruction + 3]],
            "div: Arithmetic Error: division resulted in NaN, division by 0!")

  if (std::isinf(film[instructions[currentInstruction + 3]]))
  {
    fprintf(stderr, "\ndiv: Arithmetic Error: division by zero!\n");
    exitCode = -1;
    goto _final;
  }
...
_squareRoot:
  film[instructions[currentInstruction + 2]] =
      sqrt(film[instructions[currentInstruction + 1]]);

  CHECK_NAN(film[instructions[currentInstruction + 2]],
            "sqrt: Arithmetic Error: square root of a negative number resulted "
            "in NaN!")
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_sin:
  film[instructions[currentInstruction + 2]] =
      sin(film[instructions[currentInstruction + 1]]);
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];

_cos:
  film[instructions[currentInstruction + 2]] =
      cos(film[instructions[currentInstruction + 1]]);
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];

_tan:
  film[instructions[currentInstruction + 2]] =
      tan(film[instructions[currentInstruction + 1]]);
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];

_asin:
  film[instructions[currentInstruction + 2]] =
      asin(film[instructions[currentInstruction + 1]]);

  CHECK_NAN(
      film[instructions[currentInstruction + 2]],
      "asin: Arithmetic Error: asin input outside [-1, 1] resulted in NaN!")
...
_acos:
  film[instructions[currentInstruction + 2]] =
      acos(film[instructions[currentInstruction + 1]]);

  CHECK_NAN(
      film[instructions[currentInstruction + 2]],
      "acos: Arithmetic Error: acos input outside [-1, 1] resulted in NaN!")
...
_atan2:
  film[instructions[currentInstruction + 3]] =
      atan2(film[instructions[currentInstruction + 1]],
            film[instructions[currentInstruction + 2]]);
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_pow: {
  ...
  CHECK_NAN(film[instructions[currentInstruction + 3]],
      "pow: Arithmetic Error: exponentiation resulted in NaN!")

  if (std::isinf(film[instructions[currentInstruction + 3]])) {
    fprintf(stderr, "\npow: Arithmetic Error: exponentiation resulted in infinity (likely 0 raised to a negative power)!\n");
    exitCode = -1;
    goto _final;
  }
}
...
_mod:
  film[instructions[currentInstruction + 3]] =
      modulo(film[instructions[currentInstruction + 1]],
             film[instructions[currentInstruction + 2]]);

  CHECK_NAN(film[instructions[currentInstruction + 3]],
            "mod: Arithmetic Error: modulo division by zero resulted in NaN!")
...
_notOp:
  film[instructions[currentInstruction + 2]] =
      (film[instructions[currentInstruction + 1]] == 0) ? 1 : 0;
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];

_andOp:
  film[instructions[currentInstruction + 3]] =
      (film[instructions[currentInstruction + 1]] != 0 &&
       film[instructions[currentInstruction + 2]] != 0.0)
          ? 1.0
          : 0.0;
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_orOp:
  film[instructions[currentInstruction + 3]] =
      (film[instructions[currentInstruction + 1]] != 0.0 ||
       film[instructions[currentInstruction + 2]] != 0.0)
          ? 1.0
          : 0.0;
  currentInstruction += STEP;
...
_jumpIf:
  currentInstruction =
      (film[instructions[currentInstruction + 1]] != 0)
          ? static_cast<int>(film[instructions[currentInstruction + 2]]) * 4
          : currentInstruction + STEP;
  goto *labels[instructions[currentInstruction]];

_print: {
  char *toPrint = getStringFrom(instructions[currentInstruction + 1],
...
_printNumbers:
  for (int i = instructions[currentInstruction + 1];
       i < (instructions[currentInstruction + 1] +
            instructions[currentInstruction + 2]);
       i++) {
    addDoubleToBuffer(film[i]);
    if (i < (instructions[currentInstruction + 1] +
             instructions[currentInstruction + 2]) -
                1)
      addToBuffer(" ");
  }
...
_copyFrom:
  film[instructions[currentInstruction + 2]] =
      film[static_cast<int>(film[instructions[currentInstruction + 1]])];
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_jump:
  currentInstruction =
      static_cast<int>(film[instructions[currentInstruction + 1]]) * 4;
  goto *labels[instructions[currentInstruction]];
...
_floor:
  film[instructions[currentInstruction + 2]] =
      std::floor(film[instructions[currentInstruction + 1]]);
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];

_round:
  film[instructions[currentInstruction + 2]] =
      std::round(film[instructions[currentInstruction + 1]]);
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];

_ceil:
  film[instructions[currentInstruction + 2]] =
      std::ceil(film[instructions[currentInstruction + 1]]);
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_newList:
  matrix.emplace_back();
  film[instructions[currentInstruction + 1]] =
      static_cast<double>(matrix.size() - 1);
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_lengthOfList: {
  LIST_ID("llen")
  film[instructions[currentInstruction + 2]] =
      static_cast<double>(matrix[lId].size());
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
}
...
_addAt: {
  ...
  if (index < 0)
    index = 0;
  if (index > static_cast<int>(matrix[lId].size()))
    index = static_cast<int>(matrix[lId].size());
...
_printVectorSeparated: {
  ...
  int separateEach =
      static_cast<int>(film[instructions[currentInstruction + 3]]);
  if (separateEach <= 0) {
  ...
  const std::vector<double> &vector = matrix[lId];
  char separator = doubleToChar(
      static_cast<int>(film[instructions[currentInstruction + 2]]));
  for (size_t i = 0; i < vector.size(); ++i) {
    addToBuffer(doubleToChar(vector[i]));
    if ((i + 1) % separateEach == 0 && i + 1 != vector.size())
...
_oppSetPosition:
  setCursorPosition(film[instructions[currentInstruction + 1]],
                    film[instructions[currentInstruction + 2]]);
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];

_oppSetTextSize:
  setTextSize(static_cast<float>(film[instructions[currentInstruction + 1]]));
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_oppIsPressed:
  film[instructions[currentInstruction + 2]] =
      isPressed(static_cast<int>(film[instructions[currentInstruction + 1]]));
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_mousePressed:
  ...
  case 2:
    ...
    break;
  default:
    cerr << "Unknown mouse button: "
         << static_cast<int>(film[instructions[currentInstruction + 1]])
         << endl;
  }
  currentInstruction += STEP;
  goto *labels[instructions[currentInstruction]];
...
_load: {
  ...
  if (destId < 0 || destId >= static_cast<int>(matrix.size())) {
    fprintf(stderr,
            "\nlload: Matrix Error: destination list index %d out of bounds "
            "(have %zu lists)!\n",
            destId, matrix.size());
    if (debugFile) {
      fprintf(stderr, "%s\nCode: %s\n",
  ...
  if (nameId < 0 || nameId >= static_cast<int>(matrix.size())) {
    fprintf(stderr,
            "\nlload: Matrix Error: filename list index %d out of bounds (have "
            "%zu lists)!\n",
            nameId, matrix.size());
    if (debugFile) {
      fprintf(stderr, "%s\nCode: %s\n",
...
_store: {
  ...
  if (srcId < 0 || srcId >= static_cast<int>(matrix.size())) {
    fprintf(stderr,
            "\nlstore: Matrix Error: data list index %d out of bounds (have "
            "%zu lists)!\n",
            srcId, matrix.size());
    if (debugFile) {
      fprintf(stderr, "%s\nCode: %s\n",
  ...
  if (nameId < 0 || nameId >= static_cast<int>(matrix.size())) {
    fprintf(stderr,
            "\nlstore: Matrix Error: filename list index %d out of bounds "
            "(have %zu lists)\n",
            nameId, matrix.size());
    if (debugFile) {
      fprintf(stderr, "%s\nCode: %s\n",
...
_final:
  ...
  std::cout << std::endl
            << "[Program finished in " << duration.count()
            << "ms with exit code: " << exitCode << "]" << std::endl;
